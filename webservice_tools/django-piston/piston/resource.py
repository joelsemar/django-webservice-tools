...
class Resource(object):
    ...
    def __call__(self, request, *args, **kwargs):
        ...
        if handler.required_fields.get(rm):
            qdict = getattr(request, rm)
            for field in handler.required_fields.get(rm):
                if not qdict.get(field):
                     return HttpResponse("%s is a required field" % field, status=404) 
