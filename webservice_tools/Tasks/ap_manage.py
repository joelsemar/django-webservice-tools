...
class ApTaskManage(object):
    ...
    def clone(self, options):
        ...
        os.system('cd /var/git && sudo -u www-data git clone git@github.com:appiction/%s.git' % project_name)
        os.system('sudo -u www-data ln -s /var/git/%s/server/%s /var/www/%s' % (project_name, server_name, server_name))
        os.system('sudo -u www-data ln -s /usr/local/lib/python2.6/dist-packages/django/contrib/admin/media/ /var/www/%s/static/admin-media' % server_name)
    ...
    def update(self, options):
        os.system('sudo -u www-data git reset --hard && sudo -u www-data git pull && sudo /etc/init.d/apache2 restart')
