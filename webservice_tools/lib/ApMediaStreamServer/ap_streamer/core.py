...
class FileWriter(Element):
    ...
    def __init__(self, path='', append=False, replace=False, chunks_per_file=None):
        ...
        if chunks_per_file and (not isinstance(chunks_per_file, int) or chunks_per_file < 0):
            raise ApMediaError('chunks_per_file should be an int greater than 0')
    ...
    def set_path(self, path):
        if self.path:
            raise ApMediaError("The path on this element has already been set")
        if self.chunks_per_file and '%s' not in path:
            ...
            if result[1] == '.':
                ...
                path += '%s'
        if path.count('%s') > 1:
            raise ApMediaError('Invalid Path, limit string interpolation count to 1')
        self.path = path
    ...
    def data_received(self, data=''):
        if not self.path:
            raise ApMediaError("The path must be set before using this element")
        if not self.file or self.replace:
            self.new_file()
        if data:
